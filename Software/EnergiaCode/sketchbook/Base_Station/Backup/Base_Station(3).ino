...
const uint8_t POWER_LENGTH = 16;
const uint8_t PACKET_LENGTH = ID_LENGTH + POWER_LENGTH;
const uint8_t TIMESTAMP_LENGTH = 22;
uint8_t closeFile = 0;
uint8_t block[512] = {0};
...
*/
    ...
    attachInterrupt(PUSH1, CloseFile, FALLING); // Interrupt is fired whenever button is pressed
...
void loop() {
	...
	if(Radio.CheckReceiveFlag()) {
		GetTimeStamp((char*) blockptr);
		blockptr += TIMESTAMP_LENGTH;
		Radio.ReceiveData(blockptr);
		blockptr += PACKET_LENGTH;
		bytesWritten += TIMESTAMP_LENGTH + PACKET_LENGTH;

   		if((bytesWritten + TIMESTAMP_LENGTH + PACKET_LENGTH) > 512) {
			rc = FatFs.open("DATA.txt");					// Open data file
	   		if(rc) { die(rc); }								// plz dnt die
	   		FatFs.lseek(writeBlock);						// seek to next write block
	   		*blockptr     = 32;
	   		*(blockptr+1) = 32;
	   		*(blockptr+2) = 32;
	   		*(blockptr+3) = 32;
	   		*(blockptr+4) = 13;
	   		*(blockptr+5) = 10;
	   		blockptr -= bytesWritten;						// reset block ptr to beginning
	        rc = FatFs.write(blockptr, 512, &bw);	// write block
	        FatFs.close();																			
	        writeBlock += 512;
	        memset(block, 0, bytesWritten);
	        bytesWritten = 0;
...
void die (int pff_err	/* FatFs return value */) {
