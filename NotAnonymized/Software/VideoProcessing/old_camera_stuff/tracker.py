...
class Tracker():
    def __init__(self, filename, outputpath, video_params, ttl=2, debug=False):
        ...
        self.timestring = self.filename.split("/")[-1].split(".")[-2]
        self.clipstart = datetime.strptime(self.timestring, "%Y-%m-%d %H %M %S-P")

        #open up the csv file that will get the summary info
        self.outputfile = open(os.path.normpath(self.outputpath)+"/"+self.timestring+".csv","w")
        self.columns = ["id","seconds","frames","first_frame","last_frame","start_time","end_time","start_time_offset_seconds","end_time_offset_seconds","up_moves","down_moves","left_moves","right_moves","up","down","left","right","percent_right","percent_left","left_to_right","right_to_left"]
        self.logEntry(None)
    ...
    def summarizeTrack(self, objectID):
        ...
        midline = int(self.video_params["width"]/2)
        firstFrame = track[0]["frame"]
        lastFrame = track[len(track)-1]["frame"]
        ...
        for (i,entry) in enumerate(track):
            ...
            if point[0] > midline:
                rights += 1
                if i > 0 and track[i-1]["center"][0] < midline:
                    LTR += 1
            else:
                lefts += 1
                if i > 0 and track[i-1]["center"][0] > midline:
        ...
        return {"id":objectID, 
                ...
                "percent_right" : float(rights)*100.0/float(len(track)),
                "percent_left"  : float(lefts)*100.0/float(len(track)),
                "left_to_right" : LTR,
                "right_to_left" : RTL }
    ...
    def findObjects(self, frame, minarea):
        ...
        cv2.line(frame,(midline,0),(midline,height),(255,0,0),1)
