...
P4OUT &= ~BIT0;
    ...
    //     FRAM_buffer[shamt + 4] = event_count;
    //     FRAM_buffer[shamt + 5] = ins;
    //     FRAM_buffer[shamt + 6] = outs;
    //     FRAM_buffer[shamt + 7] = passbys;
    //     FRAM_buffer[shamt + 8] = restarts;
    //     FRAM_buffer[shamt + 9] = other;

    //     if(FRAM_buffer[0] >= 1 && (P1IN & BIT2))
    //     {
    //       //make pkt to send
    //       ref_idx = cur_idx;
    ...
    //       tx_buffer[0] = 57;   //key for receiver
    //       for(int e = 1; e <= (NUMEVENTS*EVENTBYTES); e++)
    //       {
    //         shamt = ref_idx * EVENTBYTES;
    //         tx_buffer[e] =  FRAM_buffer[shamt + ((e-1) % EVENTBYTES) + 1];
    //         if((e % EVENTBYTES) == 0)
    //         {
    //           ref_idx = (ref_idx + 2) % NUMEVENTS;
    //         }
    //       }
    ...
    //     delay_timerB0(TIMER_HZ/200);   // 1s = 4096, /8 seem to work well here.
    //     cur_idx = (cur_idx + 1) % NUMEVENTS;
    //     enable_detectorISRs();
    //   }
    ...
    if(timerA0_flag) //inter-event interval or timerA1_flag
    {
        if((P1IN & BIT2))
        {
            ...
            tx_buffer[2] = iet_ct;  //inter-event time count
            tx_buffer[3] = 6;
            //send to pkt
            send_radio_pkt(tx_buffer, TX_BUFFER_SIZE);
...
void evaluate_event()
{
  ...
  if(dist_st > 81 && ch1_len > 774 && dist_end <= 896)
  {
    ins++;
  }
  else if(dist_st <= 81 && ch1_end > 214 && ch2_len > 1364)
  {
    outs++;
  }
  else if((dist_st <= 81 && ch1_end <= 214) || (dist_st <= 81 && ch1_end > 214 && ch2_len <= 1364)
           || (dist_st > 81 && ch1_len <= 774) || (dist_st > 81 && dist_end > 896))
  {
    passbys++;
  }
  else
